...
func TestReadJSONLine(t *testing.T) {
	...
	line, err := ReadJSONLine(t.Context(), reader, func() error { return errClosed })
	if err != nil {
	...
	if _, err := ReadJSONLine(t.Context(), bufio.NewReader(bytes.NewBufferString("partial")), func() error { return errClosed }); !errors.Is(err, io.EOF) {
		t.Fatalf("ReadJSONLine(partial EOF) error = %v, want io.EOF", err)
	}
	if _, err := ReadJSONLine(t.Context(), nil, func() error { return errClosed }); !errors.Is(err, errClosed) {
		t.Fatalf("ReadJSONLine(nil) error = %v, want %v", err, errClosed)
	}
}

func TestReadJSONLineCanceled(t *testing.T) {
	t.Parallel()
