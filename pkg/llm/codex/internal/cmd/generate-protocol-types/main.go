...
const (
	maxHTTPSchemaBytes = 32 << 20

	generatedSchemaFilename    = "codex_app_server_protocol.v2.schemas.json"
	generatedSchemaSourceLabel = "codex app-server generate-json-schema --experimental"
)

func main() {
	schemaPath := flag.String("schema", "", "path or URL to protocol JSON schema; defaults to running codex app-server generate-json-schema")
	codexBin := flag.String("codex-bin", "codex", "codex binary used when -schema is omitted")
	...
	input, err := loadSchemaInput(*schemaPath, *codexBin)
	if err != nil {
	...
	generated, err := newGenerator(parsed.Definitions).generate(input.label, *packageName, input.details...)
	if err != nil {
...
func sanitizeGeneratedCommentLine(line string) string {
...
func (g *generator) generate(schemaPath, packageName string, sourceDetails ...string) ([]byte, error) {
	...
	fmt.Fprintf(&out, "// Code generated by go generate ./pkg/llm/codex; DO NOT EDIT.\n")
	fmt.Fprintf(&out, "// Source: %s\n", schemaSourceLabel(schemaPath))
	for _, detail := range sourceDetails {
		fmt.Fprintf(&out, "// %s\n", sanitizeGeneratedCommentLine(detail))
	}
	out.WriteByte('\n')
