...
func main() {
	if os.Getenv("RUN_REAL_CLAUDE_TESTS") != "1" {
		fmt.Fprintln(os.Stderr, "quick_start: set RUN_REAL_CLAUDE_TESTS=1 to run against the real claude CLI.")
		return
	}

	ctx := context.Background()

	for msg, err := range claude.Query(ctx, "What is the capital of France?", &claude.Options{MaxTurns: 1}) {
		if err != nil {
			log.Fatal(err)
		}
