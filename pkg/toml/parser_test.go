...
func TestUnmarshal_ArraySeparatorValidation(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "double comma", in: "x = [1,,2]\n"},
		{name: "missing comma bool", in: "x = [true false]\n"},
		{name: "missing comma ints", in: "x = [ 1 2 3 ]\n"},
		{name: "only comma", in: "x = [,]\n"},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
