...
func (g *generator) generate(schemaPath, packageName string) ([]byte, error) {
	...
	g.rawUnions = packageRawUnionOverrides(packageName)
	g.rawDecodes = packageRawDecodeOverrides(packageName)
...
func packageRawUnionOverrides(packageName string) map[string]struct{} {
	if packageName != "codexappserver" {
		return nil
	}
	return map[string]struct{}{
		// AskForApproval has a hand-written compatibility constructor surface in
		// public_types.go. Keep the generated root raw so those exported helper
		// aliases remain the authoritative public API for this schema family.
		"AskForApproval": {},
	}
}

func packageRawDecodeOverrides(packageName string) map[string]struct{} {
	if packageName != "codexappserver" {
		return nil
	}
	return map[string]struct{}{
		// ThreadItem is consumed by SDK routing helpers that intentionally keep
		// turn-stream item payloads raw before applying higher-level decoding.
		"ThreadItem": {},
	}
}
