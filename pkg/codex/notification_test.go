	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			case NotificationMethodItemCompleted:
				...
				asValue, ok, err := DecodeNotificationAs[ItemCompletedNotification](tt.notification, NotificationMethodItemCompleted)
				if err != nil || !ok {
					t.Fatalf("DecodeNotificationAs() = (%#v, %v, %v), want success", asValue, ok, err)
				}
			...
			case NotificationMethodTurnCompleted:
				...
				asValue, ok, err := DecodeNotificationAs[TurnCompletedNotification](tt.notification, NotificationMethodTurnCompleted)
				if err != nil || !ok {
					t.Fatalf("DecodeNotificationAs() = (%#v, %v, %v), want success", asValue, ok, err)
				}
				if diff := gocmp.Diff(got, asValue); diff != "" {
...
func TestNotificationConvenienceHelpersCoverKnownMethods(t *testing.T) {
	...
	tests := map[string]notificationConvenienceCase{
		...
		"thread/settings/updated": newNotificationConvenienceCase(t, NotificationMethodThreadSettingsUpdated, ThreadSettingsUpdatedNotification{
			ThreadID: "thread",
			ThreadSettings: ThreadSettings{
				ApprovalPolicy:    AskForApprovalValueNever,
				ApprovalsReviewer: ApprovalsReviewerUser,
				CollaborationMode: CollaborationMode{
					Mode:     ModeKindDefaultValue,
					Settings: Settings{Model: "gpt-5.5"},
				},
				Cwd:           "/tmp",
				Model:         "gpt-5.5",
				ModelProvider: "openai",
				SandboxPolicy: DangerFullAccessSandboxPolicy{Type: "dangerFullAccess"},
			},
		}, DecodeThreadSettingsUpdatedNotification, Notification.ThreadSettingsUpdated),
...
func TestDecodeNotificationMethodMismatchAndMalformedParams(t *testing.T) {
	...
	mismatch, ok, err := DecodeNotificationAs[ErrorNotification](mismatchedNotification, NotificationMethodError)
	if err != nil {
		t.Fatalf("DecodeNotificationAs() mismatch error = %v", err)
	}
	if ok {
		t.Fatalf("DecodeNotificationAs() mismatch ok = true, want false")
	}
	if diff := gocmp.Diff(ErrorNotification{}, mismatch); diff != "" {
		t.Fatalf("DecodeNotificationAs() mismatch value (-want +got):\n%s", diff)
	}
